 ...
 *     </table>
 * */
 public class ServerBodyDecoder extends MessageToMessageDecoder<BinaryPacket> {
	private static final Logger log =LoggerFactory.getLogger(ServerBodyDecoder.class);
	private boolean incred=false;//是否已自增
    private ControlProxyFactory controlProxyFactory;
    public ServerBodyDecoder(ControlProxyFactory controlProxyFactory){
...
//        ByteBuf content=BinaryCodecApi.decodeContent(packet, ctx);
        ...
        int cmd=result.readInt();
        ControlMethodProxy controlMethodProxy =controlProxyFactory.newControlMethodProxy(cmd,new ByteDataBuffer(result));
        if(controlMethodProxy !=null){
            out.add(controlMethodProxy);
        }
        content.release();
        //incred=false;
    }
